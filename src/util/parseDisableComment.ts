import { ID_LENGTH, UPDATE_COMMAND } from './constants.js';
    ...
    (rule) => {
      ...
      if (!comment.rules.includes(rule)) {
        ...
        if (errorOnUnusedRules) {
          validationErrors.push({
            message: `Rule ${rule} in legacy comment is not in the actual lint disable list. Run \`${UPDATE_COMMAND}\` to remove it.`,
            location: {
              file: comment.file,
              line: comment.startLine,
            },
